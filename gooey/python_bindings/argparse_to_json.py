...
import argparse
import os
from argparse import (_CountAction, _HelpAction, _StoreConstAction,
...
VALID_WIDGETS = ('FileChooser', 'MultiFileChooser', 'FileSaver', 'DirChooser',
                 'DateChooser', 'TextField', 'Dropdown', 'Counter',
                 'RadioGroup', 'CheckBox', 'MultiDirChooser')
...
{'siege': {'command': 'siege', 'display_name': 'Siege', 'contents': []}}
...
def convert(parser):
    ...
    if has_subparsers(actions):
        ...
        layout_data = OrderedDict([('primary', {
            'command': None,
            'contents': process(parser, widget_dict)
        })])

    return {'layout_type': layout_type, 'widgets': layout_data}
...
def process(parser, widget_dict):
    ...
    base_actions = [
        ...
        if action not in group_options and action.dest != 'help'
    ]
...
def is_required(action):
    ...
    return not action.option_strings and not isinstance(
        action, _SubParsersAction) or action.required == True
...
def is_standard(action):
  ...
  """
    ...
    return (not action.choices and not isinstance(action, _CountAction)
...
def is_default_progname(name, subparser):
    return subparser.prog == '{} {}'.format(
        os.path.split(sys.argv[0])[-1], name)
...
def as_json(action, widget, required):
    if widget not in VALID_WIDGETS:
        raise UnknownWidgetType(
            'Widget Type {0} is unrecognized'.format(widget))
