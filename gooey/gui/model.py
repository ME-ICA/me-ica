...
class MyWidget(object):
    ...
    def from_dict(cls, data):
        ...
        details = data['data']
        return cls(data['type'], maybe_unpack(details, 'display_name'),
...
class MyModel(object):
  ...
  '''
    ...
    def is_required_section_complete(self):
        completed_values = [
            _f for _f in [arg.value for arg in self.required_args] if _f
        ]
    ...
    def build_command_line_string(self):
        ...
        cmd_string = ' '.join([
            _f for _f in chain(required_args, optional_args, position_args)
            if _f
        ])
        if self.layout_type == 'column':
            cmd_string = '{} {}'.format(
                self.argument_groups[self.active_group].command, cmd_string)
        return '{} --ignore-gooey {}'.format(self.build_spec['target'],
                                             cmd_string)
    ...
    def group_arguments(self, widget_list):
        ...
        return list(map(self.to_object, required_args)), list(
            map(self.to_object, optional_args))
    ...
    def to_object(self, data):
        details = data['data']
        return MyWidget(data['type'], self.maybe_unpack(
            details, 'display_name'), self.maybe_unpack(details, 'help'),
