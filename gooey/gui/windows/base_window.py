...
class BaseWindow(wx.Frame):
  ...
  '''
    ...
    def set_display_font_style(self, style):
        ...
        self.runtime_display.set_font_style(wx.MODERN if style == 'monospace'
                                            else wx.DEFAULT)
    ...
    '''
        ...
        for panel in [
                self, self.head_panel, self.foot_panel, self.config_panel
        ]:
    ...
    def show_missing_args_dialog(self):
        self.show_dialog(
            _('error_title'), _('error_required_fields'), wx.ICON_ERROR)

    def confirm_exit_dialog(self):
        result = self.show_dialog(
            _('sure_you_want_to_exit'), _('close_program'), wx.YES_NO)
        return result == YES

    def confirm_stop_dialog(self):
        result = self.show_dialog(
            _('sure_you_want_to_stop'), _('stop_task'), wx.YES_NO)
        return result == YES
