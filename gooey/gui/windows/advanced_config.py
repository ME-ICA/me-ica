...
class WidgetContainer(wx.Panel):
  ...
  '''
    ...
    def layout(self):
        ...
        if self.title:
            self.container.Add(
                wx_util.h0(self, self.title), 0, wx.LEFT | wx.RIGHT, PADDING)
            self.container.AddSpacer(30)

        if self.widgets:
            self.container.Add(
                wx_util.h1(self, self.section_name), 0, wx.LEFT | wx.RIGHT,
                PADDING)
...
class ConfigPanel(ScrolledPanel):
    def __init__(self, parent, req_cols=1, opt_cols=3, title=None, **kwargs):
        ...
        self.required_section = WidgetContainer(self,
                                                i18n._("required_args_msg"))
        self.optional_section = WidgetContainer(self,
                                                i18n._("optional_args_msg"))
