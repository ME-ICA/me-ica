...
class BaseChooser(WidgetPack):
    ...
    def build(self, parent, data, choices=None):
        ...
        self.button = wx.Button(
            self.parent, label=self.button_text, size=(73, 23))
...
class BaseFileChooser(BaseChooser):
    ...
    def on_button(self, evt):
        ...
        result = (self.get_path(dlg) if dlg.ShowModal() == wx.ID_OK else None)
        if result:
...
class MultiFileSaverPayload(BaseMultiFileChooser):
    def __init__(self, *args, **kwargs):
        BaseMultiFileChooser.__init__(self, build_dialog(
            wx.FD_MULTIPLE, False))
...
class MultiDirChooserPayload(BaseMultiFileChooser):
    ...
    def __init__(self, *args, **kwargs):
        BaseMultiFileChooser.__init__(self,
                                      MultiDirChooserPayload.MyMultiDirChooser)
...
class DirDialog(wx.DirDialog):
    def __init__(self, parent, *args, **kwargs):
        wx.DirDialog.__init__(
            self, parent, 'Select Directory', style=wx.DD_DEFAULT_STYLE)
...
FileSaverPayload = build_subclass('FileSaverPayload',
                                  staticmethod(
                                      build_dialog(
                                          wx.FD_SAVE,
                                          False,
                                          defaultFile="Enter Filename")))
FileChooserPayload = build_subclass('FileChooserPayload',
                                    staticmethod(build_dialog(wx.FD_OPEN)))
