...
if __name__ == '__main__':
    ...
    exec(compile(open(selfuncfile).read(), selfuncfile, 'exec'))
...
def dice(A, B):
    denom = np.array(
        A != 0, dtype=np.int).sum(0) + (np.array(B != 0, dtype=np.int).sum(0))
    if denom != 0:
...
def spatclust(data,
    ...
    if data is not None and len(
            np.squeeze(data).shape) > 1 and dindex + tindex == 0:
        addopts = "-doall"
    else:
        addopts = "-1dindex %s -1tindex %s" % (str(dindex), str(tindex))
    os.system(
        '3dmerge -overwrite %s -dxyz=1  -1clust 1 %i -1thresh %.02f -prefix __clout.nii.gz %s'
        % (addopts, int(csize), float(thr), infile))
...
def t2smap(catd, mask, tes):
    ...
    out = np.squeeze(unmask(t2s, mask)), np.squeeze(unmask(s0, mask)), unmask(
        t2s_fit, mask)
...
def t2sadmap(catd, mask, tes):
    ...
    for ne in range(1, Ne + 1):
        ...
        #Do Log Linear fit
        B = np.reshape(np.abs(echodata[:, :ne]) + 1, (Nm,
                                                      (ne) * nt)).transpose()
        B = np.log(B)
...
def getelbow2(ks, val=False):
    ...
    ds = np.array(
        [(ks[nk - 5 - ii - 1] >
          ks[nk - 5 - ii:nk].mean() + 2 * ks[nk - 5 - ii:nk].std())
         for ii in range(nk - 5)][::-1],
        dtype=np.int)
    dsum = []
    c_ = 0
    for d_ in ds:
...
def eimask(dd, ees=None):
    if ees == None: ees = list(range(dd.shape[1]))
...
def idwtmat(mmix_wt, cAl):
    ...
    lt = len(
        pywt.idwt(
            mmix_wt[0, :cAl], mmix_wt[0, cAl:], 'db2', correct_size=True))
...
def tedpca(ste=0, mlepca=True):
    ...
    if not os.path.exists(pcastate_fn):
        ...
        eimum = np.array(
            np.squeeze(unmask(eimum.prod(1), mask)), dtype=np.bool)
        vTmix = v.T
        ...
        none, ctb, betasv, v_T = fitmodels_direct(
            catd, v.T, eimum, t2s, tes, mmixN=vTmixN, full_sel=False)
    ...
    kappa_thr = np.average(
        sorted([fmin, getelbow(kappas, True) / 2, fmid]), weights=[kdaw, 1, 1])
    rho_thr = np.average(
        sorted([fmin, getelbow2(rhos, True) / 2, fmid]), weights=[rdaw, 1, 1])
    if int(kdaw) == -1:
    ...
    if options.stabilize:
        pcscore = (
            np.array(ctb[:, 1] > kappa_thr, dtype=np.int) +
            np.array(ctb[:, 2] > rho_thr, dtype=np.int) +
            np.array(ctb[:, 3] > eigelb, dtype=np.int)) * np.array(
                ctb[:, 3] > spmin, dtype=np.int) * np.array(
                    spcum < 0.95, dtype=np.int) * np.array(
                        ctb[:, 2] > fmin, dtype=np.int) * np.array(
                            ctb[:, 1] > fmin, dtype=np.int) * np.array(
                                ctb[:, 1] != F_MAX, dtype=np.int) * np.array(
                                    ctb[:, 2] != F_MAX, dtype=np.int)
    else:
        pcscore = (np.array(ctb[:, 1] > kappa_thr, dtype=np.int) +
                   np.array(ctb[:, 2] > rho_thr, dtype=np.int) +
                   np.array(ctb[:, 3] > eigelb, dtype=np.int)) * np.array(
                       ctb[:, 3] > spmin, dtype=np.int) * np.array(
                           ctb[:, 1] != F_MAX, dtype=np.int) * np.array(
    ...
    pcsel = pcscore > 0
    pcrej = np.array(
        pcscore == 0, dtype=np.int) * np.array(
            ctb[:, 3] > spmin, dtype=np.int) > 0
    """
kdaw=15
rdaw=5
...
np.array(pcscore>0).sum()
    ...
    nc = s[pcsel].shape[0]
    print(pcsel)
    print("--Selected %i components. Minimum Kappa=%0.2f Rho=%0.2f" %
          (nc, kappa_thr, rho_thr))

    dd = ((
        dd.T - dd.T.mean(0)) / dd.T.std(0)).T  #Variance normalize timeseries
...
def tedica(dd, cost):
    ...
    icanode = mdp.nodes.FastICANode(
        ...
        approach='symm',
        g=cost,
        fine_g=options.finalcost,
        primary_limit=climit * 100,
        limit=climit,
        verbose=True)
    icanode.train(dd)
...
def write_split_ts(data, comptable, mmix, suffix=''):
    ...
    betas = fmask(
        get_coeffs(unmask((mdata.T - mdata.T.mean(0)).T, mask), mask, mmix),
        mask)
    ...
    if len(acc) != 0:
        niwrite(
            unmask(betas[:, acc].dot(mmix.T[acc, :]), mask), aff,
            '_'.join(['hik_ts', suffix]) + '.nii')
    if len(midk) != 0:
        niwrite(
            unmask(midkts, mask), aff, '_'.join(['midk_ts', suffix]) + '.nii')
    if len(rej) != 0:
        niwrite(
            unmask(lowkts, mask), aff, '_'.join(['lowk_ts', suffix]) + '.nii')
    niwrite(
        unmask(fmask(data, mask) - lowkts - midkts, mask), aff,
...
def writefeats(cbetas, comptable, mmix, suffix=''):
    ...
    zfac = 1. / (mmix.shape[0] - len(acc) - 1) * (
        noise**2).sum(-1)  #noise scaling
...
def computefeats2(data, mmix, mask, normalize=True):
    ...
    data = data[mask]
    data_vn = (data - data.mean(axis=-1)[:, np.newaxis]
               ) / data.std(axis=-1)[:, np.newaxis]
    ...
    if normalize:
        ...
        data_Z = (((data_Z.T - data_Z.mean(0)[:, np.newaxis]) /
                   data_Z.std(0)[:, np.newaxis]) +
                  (data_Z.mean(0) / data_Z.std(0))[:, np.newaxis]).T
    return data_Z
...
def gscontrol_raw(dtrank=4):
    ...
    Lmix = np.array([
        lpmv(0, vv, np.linspace(-1, 1, OCcatd.shape[-1]))
        for vv in range(dtrank)
    ]).T
    ...
    tsoc_nogs = dat - np.dot(
        np.atleast_2d(sol[0][dtrank]).T, np.atleast_2d(
...
def gscontrol_mmix():
	...
	"""
    dat = (
        OCcatd[Gmask] - Gmu[Gmask][:, np.newaxis]) / Gstd[mask][:, np.newaxis]
    ...
    bold_noT1gs = bold_ts - np.dot(
        np.linalg.lstsq(glsig.T, bold_ts.T)[0].T, glsig)
    niwrite(
    ...
    mmixnogs_norm = (
        mmixnogs - mmixnogs_mu[:, np.newaxis]) / mmixnogs_std[:, np.newaxis]
...
if __name__ == '__main__':
    ...
    parser.add_option(
        ...
        dest='ste',
        help=
        "Source TEs for models. ex: -ste 2,3 ; -ste 0 for all, -1 for opt. com. Default -1.",
        default=0 - 1)
    parser.add_option(
        '',
        "--combmode",
    ...
    if options.mixm != None:
        try:
            os.system('cp %s %s/meica_mix.1D; cp %s %s/%s' %
                      (options.mixm, dirname, options.mixm, dirname,
                       os.path.basename(options.mixm)))
        except:
            pass
    if options.ctab != None:
        try:
            os.system('cp %s %s/comp_table.txt; cp %s %s/%s' %
                      (options.mixm, dirname, options.mixm, dirname,
                       os.path.basename(options.mixm)))
        except:
            pass
    ...
    if options.mixm == None:
        ...
        seldict, comptable, betas, mmix = fitmodels_direct(
            catd, mmix_orig, mask, t2s, tes, options.fout, reindex=True)
        np.savetxt('meica_mix.1D', mmix)
        if 'GROUP0' in sys.argv:
            group0_flag = True
        else:
            group0_flag = False
        acc, rej, midk, empty = selcomps(
            seldict,
            knobargs=args,
            debug=debug_mode,
        ...
        eimum = np.array(
            np.squeeze(unmask(np.array(eim, dtype=np.int).prod(1), mask)),
            dtype=np.bool)
        seldict, comptable, betas, mmix = fitmodels_direct(
            catd, mmix_orig, mask, t2s, tes, options.fout)
        if options.ctab == None:
            acc, rej, midk, empty = selcomps(
                seldict,
                knobargs=args,
