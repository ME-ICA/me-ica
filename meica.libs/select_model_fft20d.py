...
def fitmodels_direct(catd,
    ...
    for i in range(nc):
        ...
        varex_norm[i] = (
            unmask(WTS, mask)[t2s != 0][:, i]**2).sum() / totvar_norm * 100.
        ...
        wtsZ[np.abs(wtsZ) > Z_MAX] = (Z_MAX * (
            np.abs(wtsZ) / wtsZ))[np.abs(wtsZ) > Z_MAX]
        Z_maps[:, i] = wtsZ
        ...
        Kappas[i] = np.average(
            F_R2, weights=np.abs(np.squeeze(unmask(wtsZ, mask)[t2s != 0]**2.)))
        Rhos[i] = np.average(
            F_S0, weights=np.abs(np.squeeze(unmask(wtsZ, mask)[t2s != 0]**2.)))
    ...
    if full_sel:
        for i in range(nc):
            ...
            os.system(
                '3drefit -sublabel 0 PSC -sublabel 1 F_R2  -sublabel 2 F_SO -sublabel 3 Z_sn %s 2> /dev/null > /dev/null'
                % ccname)
            ...
            os.system(
                "3dcalc -overwrite -a %s[1..2] -expr 'a*step(a-%i)' -prefix .fcl_in.nii.gz -overwrite"
                % (ccname, fmin))
            os.system(
                '3dmerge -overwrite -dxyz=1 -1clust 1 %i -doall -prefix .fcl_out.nii.gz .fcl_in.nii.gz'
                % (csize))
            ...
            Br_clmaps_R2[:, i] = spatclust(
                rankvec(tsoc_Babs[:, i]), mask, csize,
                max(tsoc_Babs.shape) - countsigFR2, head, aff)
            Br_clmaps_S0[:, i] = spatclust(
                rankvec(tsoc_Babs[:, i]), mask, csize,
                max(tsoc_Babs.shape) - countsigFS0, head, aff)
        ...
        if debugout or ('DEBUGOUT' in args):
            ...
            import zlib
            try:
                os.system('mkdir compsel.debug')
            except:
                pass
            selvars = [
                'Kappas', 'Rhos', 'WTS', 'varex', 'Z_maps', 'Z_clmaps',
                'F_R2_clmaps', 'F_S0_clmaps', 'Br_clmaps_R2', 'Br_clmaps_S0',
                'PSC'
            ]
            for vv in selvars:
...
def fft_variance(fproj_arr, fproj_arr_val, A, B):
    ...
    fproj_sel_A = (andb([fproj_sel_T[0] > 0,
                         fproj_sel_T[1] < 0.05]) == 2).reshape(
                             mask.shape[0:2])
    fproj_sel_B = (andb([fproj_sel_T[0] < 0,
                         fproj_sel_T[1] < 0.05]) == 2).reshape(
                             mask.shape[0:2])
    return fproj_arr_val[fproj_sel_A.flatten()].sum(0), fproj_arr_val[
        fproj_sel_B.flatten()].sum(0)
...
def selcomps(seldict,
	...
	"""
    if knobargs != '':
        for knobarg in ''.join(knobargs).split(','):
            exec(knobarg)
    """
    ...
    for ii in ncl:
        dice_FR2 = dice(
            unmask(Br_clmaps_R2[:, ii], mask)[t2s != 0], F_R2_clmaps[:, ii])
        dice_FS0 = dice(
            unmask(Br_clmaps_S0[:, ii], mask)[t2s != 0], F_S0_clmaps[:, ii])
    ...
    for ii in nc:
        comp_noise_sel = andb(
            [np.abs(Z_maps[:, ii]) > 1.95, Z_clmaps[:, ii] == 0]) == 2
        countnoise[ii] = np.array(comp_noise_sel, dtype=np.int).sum()
        noise_FR2_Z = np.log10(
            np.unique(F_R2_maps[unmask(comp_noise_sel, mask)[t2s != 0], ii]))
        signal_FR2_Z = np.log10(
            np.unique(
                F_R2_maps[unmask(Z_clmaps[:, ii], mask)[t2s != 0] == 1, ii]))
    ...
    tt_table[np.isinf(tt_table[:, 0]), 0] = np.percentile(
        tt_table[~np.isinf(tt_table[:, 0]), 0], 98)
    ...
    p0base = np.array(
        [(np.arange(mmix.shape[0]) - np.mean(np.arange(mmix.shape[0]))) /
         np.std(np.arange(mmix.shape[0])),
         np.ones(mmix.shape[0])])
    ...
    fdist = []
    for ii in nc:
        fproj = np.fft.fftshift(
            np.abs(np.fft.rfftn(unmask(seldict['PSC'], mask)[:, :, :, ii])))
        fproj_z = fproj.max(2)
        ...
        fdist.append(
            np.max([
                fitgaussian(fproj.max(jj))[3:].max()
                for jj in range(len(fprojr.shape))
            ]))
    ...
    Kelbowval = np.median([
        getelbow(Kappas, True),
        getelbow2(Kappas, True),
        getelbow3(Kappas, True)
    ] + list(getfbounds(ne)))
    Khighelbowval = stats.scoreatpercentile([
        getelbow(Kappas, True),
        getelbow2(Kappas, True),
        getelbow3(Kappas, True)
    ] + list(getfbounds(ne)), 75)
    KRcut = np.median(KRcutguesses)
    #only use exclusive when inclusive is extremely inclusive - double KRcut
    if getelbow2(Kappas) > KRcut * 2 and getelbow(Kappas, True) < F01:
        Kcut = getelbow(Kappas, True)
    else:
        Kcut = getelbow2(Kappas, True)
    ...
    if getelbow2(Rhos) > KRcut * 2:
        Rcut = getelbow(
            Rhos, True
        )  #consider something like min([getelbow(Rhos,True),sorted(Rhos)[::-1][KRguess] ])
    else:
        Rcut = getelbow2(Rhos, True)
    ...
    KRguess = np.setdiff1d(
        ...
        np.union1d(nc[tt_table[:, 0] < tt_lim],
                   np.union1d(
                       np.union1d(nc[spz > 1], nc[Vz > 2]), nc[andb([
                           varex > 0.5 * sorted(varex)[::-1][int(KRcut)],
                           Kappas < 2 * Kcut
                       ]) == 2])))
    ...
    rejB = ncl[andb([
        tt_table[ncl, 0] < 0, varex[ncl] > np.median(varex), ncl > KRcut
    ]) == 3]
    ...
    for ii in range(20000):
        ...
        if db.labels_.max(
        ) > 1 and db.labels_.max() < len(nc) / 6 and np.intersect1d(
                rej, nc[db.labels_ == 0]).shape[0] == 0 and np.array(
                    db.labels_ == -1, dtype=int).sum() / float(len(nc)) < .5:
            epsmap.append([
                ii,
                dice(guessmask, db.labels_ == 0),
                np.intersect1d(nc[db.labels_ == 0],
                               nc[Rhos > getelbow(Rhos_sorted, True)]).shape[0]
            ])
    ...
    if len(epsmap) != 0:
        ...
        ii = epsmap[np.argmax(epsmap[epsmap[:, 2] == np.min(epsmap[:, 2]), 1],
                              0), 0]
    ...
    if len(group0) < 2 or (len(group0) < 4
        ...
        print(
            "WARNING: Extremely limited reliable BOLD signal space. Not filtering further into midk etc."
        )
        midkfailed = True
        min_acc = np.array([])
        if len(group0) != 0:
            toacc_hi = np.setdiff1d(
                nc[andb([fdist <= np.max(fdist[group0]), Rhos < F025, Vz > -2])
                   == 3],
                np.union1d(group0,
                           rej))  #For extremes, building in a 20% tolerance
            min_acc = np.union1d(group0, toacc_hi)
            to_clf = np.setdiff1d(nc, np.union1d(min_acc, rej))
        diagstepkeys = [
            'rej', 'KRcut', 'Kcut', 'Rcut', 'dbscanfailed', 'midkfailed',
            'KRguess', 'group0', 'min_acc', 'toacc_hi'
        ]
        diagstepout = []
    ...
    if group0_only:
        return list(sorted(group0)), list(sorted(rej)), [], list(
            sorted(to_clf))
    ...
    if not dbscanfailed and len(rej) + len(group0) < 0.75 * len(nc):
        ...
        rej_supp = np.setdiff1d(
            np.setdiff1d(
                np.union1d(rej, nc[dice_table[nc, 0] <= dice_table[nc, 1]]),
                group0), group_n1)
    ...
    #Temporal features
    mmix_kurt_z = (
        mmix_kurt - mmix_kurt[group0].mean()) / mmix_kurt[group0].std()
    mmixp0_kurt_z = (
        mmixp0_kurt - mmixp0_kurt[group0].mean()) / mmixp0_kurt[group0].std()
    ...
    toacc_hi = np.setdiff1d(
        nc[andb([fdist <= np.max(fdist[group0]), Rhos < F025, Vz > -2]) == 3],
        np.union1d(group0, rej)
    )  #Tried getting rid of accepting based on SVM altogether, now using only rejecting
    toacc_lo = np.intersect1d(to_clf, nc[andb([
        spz < 1, Rz < 0, mmix_kurt_z_max < 5, Dz > -1, Tz > -1, Vz < 0,
        Kappas >= F025, fdist < 3 * np.percentile(fdist[group0], 98)
    ]) == 8])
    midk_clf, clf_ = do_svm(
        fproj_arr_val[:, np.union1d(group0, rej)].T,
        [0] * len(group0) + [1] * len(rej),
        fproj_arr_val[:, to_clf].T,
        svmtype=2)
    midk = np.setdiff1d(to_clf[andb(
        [midk_clf == 1, varex[to_clf] > np.median(varex[group0])]) == 2],
                        np.union1d(toacc_hi, toacc_lo))
    if len(
            np.intersect1d(to_clf[andb([midk_clf == 1, Vz[to_clf] > 0]) == 2],
                           toacc_hi)) == 0:
        svm_acc_fail = True
        toacc_hi = np.union1d(
            toacc_hi, to_clf[midk_clf == 0]
        )  #only use SVM to augment toacc_hi only if toacc_hi isn't already conflicting with SVM choice
    else:
        svm_acc_fail = False
    """
    ...
    veinmask = andb([t2s < scoreatpercentile(t2s[t2s != 0], 15),
                     t2s != 0]) == 2
    veinmaskf = veinmask[t2s != 0]
    veinR = np.array(
        sig_B[veinmaskf].sum(0), dtype=float) / sig_B[~veinmaskf].sum(0)
    ...
    t2s_lim = [
        stats.scoreatpercentile(t2s[t2s != 0], 50),
        stats.scoreatpercentile(t2s[t2s != 0], 80) / 2
    ]
    phys_var_zs = []
    for t2sl_i in range(len(t2s_lim)):
        ...
        veincand = fmask(
            unmask(
                andb([
                    s0[t2s != 0] < np.median(s0[t2s != 0]),
                    t2s[t2s != 0] < t2sl
                ]) >= 1, t2s != 0), mask)
        veinW[~veincand] = 0
        invein = veinW.sum(1)[fmask(
            unmask(veinmaskf, t2s != 0) * unmask(veinW.sum(1) > 1, mask),
            mask)]
    ...
    phys_art = np.setdiff1d(
        nc[andb([phys_var_z > 3.5, Kappas < minK_ign]) == 2], group0)
    phys_art = np.union1d(
        np.setdiff1d(nc[andb([
            phys_var_z > 2,
            rankvec(phys_var_z) - rankvec(Kappas) > newcest / 2, Vz2 > -1
        ]) == 3], group0), phys_art)
    #Want to replace field_art with an acf/SVM based approach instead of a kurtosis/filter one
    field_art = np.setdiff1d(
        nc[andb([mmix_kurt_z_max > 5, Kappas < minK_ign]) == 2], group0)
    field_art = np.union1d(
        np.setdiff1d(nc[andb([
            mmix_kurt_z_max > 2,
            rankvec(mmix_kurt_z_max) - rankvec(Kappas) > newcest / 2, Vz2 > 1,
            Kappas < F01
        ]) == 4], group0), field_art)
    misc_art = np.setdiff1d(
        nc[andb([(rankvec(Vz) - rankvec(Ktz)) > newcest / 2,
    ...
    if not strict_mode:
        ...
        ncl = np.union1d(ncl,
                         np.intersect1d(orphan, nc[andb([
                             Kappas > F05, Rhos < F025, Kappas > Rhos,
                             Vz3 <= -1, Vz3 > -3, mmix_kurt_z_max < 2.5
                         ]) == 6]))
        ign = np.setdiff1d(nc, list(ncl) + list(midk) + list(rej))
        orphan = np.setdiff1d(
            nc,
    ...
    if savecsdiag:
        diagstepkeys = [
            'selmodelversion', 'rej', 'KRcut', 'Kcut', 'Rcut', 'dbscanfailed',
            'KRguess', 'group0', 'dice_rej', 'rej_supp', 'to_clf', 'midk',
            'svm_acc_fail', 'toacc_hi', 'toacc_lo', 'field_art', 'phys_art',
            'misc_art', 'ncl', 'ign'
        ]
        diagstepout = []
        for ddk in diagstepkeys:
