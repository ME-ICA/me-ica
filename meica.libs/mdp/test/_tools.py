...
def get_random_mix(mat_dim = None, type = "d", scale = 1,\
    ...
    while d < 1E-3:
        ...
        d2 = min(
            mdp.utils.symeig(
                mdp.utils.mult(matmix_n.T, matmix_n), eigenvectors=0))
        d = min(d1, d2)
...
class BogusMultiNode(mdp.Node):
    ...
    def _get_train_seq(self):
        return [(self.train1, self.stop1), (self.train2, self.stop2)]
...
_spinner = itertools.cycle((' .\b\b', ' o\b\b', ' 0\b\b', ' O\b\b', ' 0\b\b',
                            ' o\b\b'))
...
class skip_on_condition(object):
    ...
    def __call__(self, f):
        ...
        def wrapped_f(*args, **kwargs):
            namespace = {'sys': sys, 'os': os, 'mdp': mdp}
