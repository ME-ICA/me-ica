...
def testPCANode_SVD():
    ...
    try:
        pca.stop_training()
        raise Exception(
            "PCANode didn't catch singular covariance matrix: degenerate")
    ...
    try:
        pca.stop_training()
        raise Exception(
            "PCANode didn't catch singular covariance matrix: undetermined")
    except mdp.NodeException:
        pass
    # now try the SVD version
    pca = mdp.nodes.PCANode(svd=True)
    ...
    assert_array_almost_equal(
        mat[:, 0].mean(axis=0), out.mean(axis=0), decimal)
    assert_array_almost_equal(mat[:, 0].std(axis=0), out.std(axis=0), decimal)
    ...
    mat *= [1E+5, 1E-3, 1E-18]
    mat -= mat.mean(axis=0)
    pca = mdp.nodes.PCANode(svd=True, reduce=True, var_abs=1E-8, var_rel=1E-30)
    pca.train(mat)
    out = pca.execute(mat)
    # check that we got the only large dimension
    assert_array_almost_equal(
        mat[:, :2].mean(axis=0), out.mean(axis=0), decimal)
    assert_array_almost_equal(mat[:, :2].std(axis=0), out.std(axis=0), decimal)
...
def testPCANode_negative_eigenvalues():
    ...
    try:
        ...
        if "Got negative eigenvalues" in str(e):
            ...
            raise Exception(
                "PCA did not catch negative eigenvalues!\n" + str(e))
