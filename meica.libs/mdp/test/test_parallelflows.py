...
def test_tasks():
    ...
    data_iterables = [[
        n.random.random((30, 10)) * n.arange(1, 11) for _ in range(6)
    ], None, [n.random.random((30, 10)) * n.arange(1, 11) for _ in range(6)]]
...
def test_multiphase_checkpoints():
    ...
    data_iterables = [[n.random.random((30, 10)) for _ in range(6)], None,
...
def test_train_purge_nodes():
    ...
    sfa_node = mdp.nodes.SFANode(input_dim=10, output_dim=8)
    sfa2_node = mdp.nodes.SFA2Node(input_dim=8, output_dim=6)
    flownode = mdp.hinet.FlowNode(
        mdp.Flow([sfa_node, mdp.nodes.IdentityNode(), sfa2_node]))
...
def test_execute_fork():
    ...
    class _test_ExecuteForkNode(mdp.nodes.IdentityNode):
        ...
        def __init__(self, input_dim=None, output_dim=None, dtype=None):
            self.n_forks = 0
            self.n_joins = 0
            super(_test_ExecuteForkNode, self).__init__(
    ...
    try:
        ...
        ], None, [n.random.random((30, 10)) for _ in range(n_chunks)], None]
        flow = parallel.ParallelFlow([
