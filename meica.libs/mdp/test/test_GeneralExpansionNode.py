...
requires_scipy = skip_on_condition("not mdp.numx_description == 'scipy'",
...
def testGeneralExpansionNode():
    ...
    assert_array_almost_equal(out[:, 0:input_dim], input, 6,
                              "incorrect constant expansion")
    assert_array_almost_equal(out[:, input_dim:2 * input_dim], input**2, 6,
                              "incorrect constant expansion")
    assert_array_almost_equal(out[:, 2 * input_dim:],
                              dumb_quadratic_expansion(input), 6,
                              "incorrect constant expansion")
    assert cen.expanded_dim(
        input_dim) == 2 * input_dim + input_dim**2, "expanded_dim failed"
    assert_array_almost_equal(
        cen.output_sizes(input_dim),
        numx.array([input_dim, input_dim, input_dim * input_dim]), 6,
        "output_sizes failed")
...
def testGeneralExpansionNode_inverse():
    ...
    assert_array_almost_equal_diff(
        input, app_input, 6, 'inversion not good enough by use_hint=True')
