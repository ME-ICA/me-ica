...
def testRBFExpansionNode():
    ...
    grid = numx.meshgrid(
        numx.linspace(-3., 4., 100), numx.linspace(-3., 4., 100))
    ...
    def check_mn_cov(rbf, real_covs):
        ...
        for i in range(n):
            ...
            # check variance
            vr = ((rrep(rrep(p, 2), 2) * grid_cov).sum(0) - numx.outer(mn, mn))
    ...
    sizes = [mdp.utils.symrand(numx.array([0.2, 0.4])) for i in range(n)]
    rbf = mdp.nodes.RBFExpansionNode(centers, sizes)
