...
def testISFANodeGivensRotations():
    ...
    covs.symmetrize()
    i = mdp.nodes.ISFANode(
        list(range(1, ncovs + 1)),
        sfa_ica_coeff=ratio,
...
def testISFANode_SFAPart():
    ...
    out = mdp.nodes.ISFANode(
        lags=1, whitened=True, sfa_ica_coeff=[1., 0.])(src)
    ...
    out = mdp.nodes.ISFANode(
        lags=1, whitened=False, sfa_ica_coeff=[1., 0.])(mix)
    max_cv = numx.diag(abs(_cov(out, src)))
    assert_array_almost_equal(max_cv, numx.ones((3, )), 5)
...
def testISFANode_ICAPart():
    ...
    out = mdp.nodes.ISFANode(
        lags=1, whitened=True, sfa_ica_coeff=[0., 1.])(src)
    max_cv = numx.diag(abs(_cov(out, src)))
    assert_array_almost_equal(max_cv, numx.ones((3, )), 5)
    # mix linearly the signals
    mix = mult(src, uniform((3, 3)) * 2 - 1)
    out = mdp.nodes.ISFANode(
        lags=1, whitened=False, sfa_ica_coeff=[0., 1.])(mix)
    max_cv = numx.diag(abs(_cov(out, src)))
    assert_array_almost_equal(max_cv, numx.ones((3, )), 5)
