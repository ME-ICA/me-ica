...
def testAddNode():
    ...
    for i in range(nnodes):
        g.add_node()
    assert len(
        g.nodes) == nnodes, "Wrong number of nodes, expected: %d, got :%d" % (
            nnodes, len(g.nodes))
    # add nodes
    g = graph.Graph()
    g.add_nodes(5)
...
def testAddEdge():
    ...
    assert len(
        g.edges
    ) == nnodes - 1, "Wrong number of edges, expected: %d, got :%d" % (
        nnodes - 1, len(g.edges))
    # the last nnodes-1 nodes should have in_degree==1,
    # and the first nnodes-1 out_degree==1
    for i in range(nnodes):
        if i > 0:
            assert nds[i].in_degree(
            ) == 1, "Wrong in_degree, expected: 1, got: %d." % nds[
                i].in_degree()
        if i < nnodes - 1:
            assert nds[i].out_degree(
            ) == 1, "Wrong out_degree, expected: 1, got: %d." % nds[
                i].out_degree()
...
def testRemoveEdge():
    ...
    g.remove_edge(ed)
    assert len(
        g.edges) == 0, "Wrong number of edges, expected: 0, got :%d" % len(
            g.edges)
    assert n0.out_degree(
    ) == 0, "Wrong out_degree, expected: 0, got: %d." % n0.out_degree()
    assert n1.in_degree(
    ) == 0, "Wrong in_degree, expected: 0, got: %d." % n1.in_degree()
