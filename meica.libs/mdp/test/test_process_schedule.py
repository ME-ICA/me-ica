...
def test_process_scheduler_shutdown():
    ...
    scheduler = parallel.ProcessScheduler(
        verbose=False, n_processes=1, source_paths=None, cache_callable=False)
    scheduler.shutdown()
...
def test_process_scheduler_order():
    ...
    scheduler = parallel.ProcessScheduler(
        verbose=False, n_processes=3, source_paths=None)
    max_i = 8
    ...
    assert n.all(
        results == n.concatenate([n.arange(0, i + 1)**2
                                  for i in range(max_i)]))
...
def test_process_scheduler_no_cache():
    ...
    scheduler = parallel.ProcessScheduler(
        verbose=False, n_processes=2, source_paths=None, cache_callable=False)
    for i in range(8):
...
def test_process_scheduler_manager():
    ...
    with parallel.ProcessScheduler(
            n_processes=2, source_paths=None) as scheduler:
        for i in range(8):
            scheduler.add_task(i, parallel.SqrTestCallable())
        results = scheduler.get_results()
    # check result
...
def test_process_scheduler_flow():
    ...
    train_iterables = [
        n.dot(mdp.numx_rand.random((5, 100, input_dim)), scale_matrix)
        for _ in range(3)
    ]
    x = mdp.numx.random.random((10, input_dim))
    with parallel.ProcessScheduler(
            verbose=False, n_processes=3, source_paths=None) as scheduler:
