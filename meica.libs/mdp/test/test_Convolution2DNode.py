...
def testConvolution2DNodeFunctionality():
    ...
    for mode in ['valid', 'same', 'full']:
        for boundary in ['fill', 'wrap', 'symm']:
            node = mdp.nodes.Convolution2DNode(
                filters,
                approach='linear',
                mode=mode,
                boundary=boundary,
...
def testConvolution2DNode_2D3Dinput():
    ...
    node = mdp.nodes.Convolution2DNode(
        filters, approach='linear', input_shape=(3, 4), output_2d=False)
    y = node.execute(x)
    ...
    node = mdp.nodes.Convolution2DNode(
        filters, approach='linear', input_shape=(3, 4), output_2d=True)
    y = node.execute(x)
    assert_equal(y.shape, (x.shape[0], 3 * 3 * 4))
    for i in range(3):
...
def testConvolution2DNode_fft():
    ...
    for mode in ['valid', 'same', 'full']:
        node = mdp.nodes.Convolution2DNode(
            filters, approach='fft', mode=mode, output_2d=False)
        y = node.execute(x)
    ...
    for mode in ['valid', 'same', 'full']:
        node_fft = mdp.nodes.Convolution2DNode(
            filters, approach='fft', mode=mode, output_2d=False)
        node_lin = mdp.nodes.Convolution2DNode(
            filters,
            approach='linear',
            mode=mode,
            boundary='fill',
...
def testConvolution2DNode_in_Flow():
    ...
    x = numx.random.random((10, 12))
    node = mdp.nodes.Convolution2DNode(
        filters, input_shape=(3, 4), output_2d=True)
    flow = mdp.Flow(
        [mdp.nodes.IdentityNode(), node,
...
def testConvolution2DNode_arguments():
    ...
    py.test.raises(mdp.NodeException, "mdp.nodes.Convolution2DNode(filters)")
