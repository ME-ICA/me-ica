...
def testFlow_save():
    ...
    dummy_file = tempfile.mktemp(
        prefix='MDP_', suffix=".pic", dir=py.test.mdp_tempdirname)
...
def testCheckpointFlow():
    ...
    mat, mix, inp = get_random_mix(mat_dim=(100, 3))
    flow = _get_default_flow(
        flow_class=mdp.CheckpointFlow, node_class=BogusNodeTrainable)
...
def testCrashRecoveryException():
    ...
    try:
        ...
        filename1 = e.dump()
        filename2 = e.dump(
            tempfile.mkstemp(prefix='MDP_', dir=py.test.mdp_tempdirname)[1])
