...
class HiNetHTMLVisitor(object):
    ...
    def hinet_css(cls):
        ...
        css_filename = os.path.join(
            os.path.split(__file__)[0], cls._CSS_FILENAME)
    ...
    def _write_node_header(self, node, type_id="node"):
        ...
        if not (type_id == "flow" or type_id == "flownode"):
            f.write('<tr><td class="dim">in-dim: %s</td></tr>' % str(
                node.input_dim))
        f.write('<tr><td>')
...
def _rect2d_switchoard_html(self):
    lines = [
        'rec. field size (in channels): %d x %d = %d' %
        ...
        'rec. field distances (in channels): ' + str(self.field_spacing_xy),
...
def _double_rect2d_switchoard_html(self):
    lines = [
        ...
        (self.field_channels_xy[0], self.field_channels_xy[1],
         self.field_channels_xy[0] * self.field_channels_xy[1]),
        '# of long row rec. fields (out channels): ' + str(
            self.long_out_channels_xy),
        'total number of receptive fields: %d' % self.output_channels,
        'channel width: %d' % self.in_channel_dim
    ]
...
def _double_rhomb2d_switchoard_html(self):
    lines = [
        'rec. field size: %d' % self.diag_field_channels,
        '# of rec. fields (out channels): %d x %d = %d' %
        (self.out_channels_xy[0], self.out_channels_xy[1],
         self.output_channels),
        'channel width: %d' % self.in_channel_dim
...
def _noise_html(self):
    return [
        'noise level: ' + str(self.noise_args[1]),
