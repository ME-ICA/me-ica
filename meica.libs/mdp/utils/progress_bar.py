...
def _progress(percent, last, style, layout):
    ...
    if style == 'bar':
        ...
        box = ''.join(
            [layout['delimiters'][0], done, todo, layout['delimiters'][1]])
        if layout['position'] == 'left':
            ...
            box = ''.join([
                '\r', layout['indent'], box[0:percent_idx], percent_s,
        ...
        if percent == 0:
            ...
            # build the time box
            tbox = ''.join([
                fmt_time(elapsed, layout['delimiters']), layout['separator'],
                fmt_time(e_t_a, layout['delimiters'])
            ])
        # compose progress info box
        if layout['position'] == 'left':
            box = ''.join(['\r', layout['indent'], percent_s, ' ', tbox])
        else:
            box = ''.join(['\r', layout['indent'], tbox, ' ', percent_s])
...
if __name__ == '__main__':
    ...
    with tempfile.TemporaryFile(mode='r+') as fl:
        ...
        for line in progressinfo(
                gen(), -10, style='timer', custom={'speed': 'last'}):
            time.sleep(1)
