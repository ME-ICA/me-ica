...
def test_write():
    ...
    out_f.truncate(0)
    out_f.seek(0)
    assert_raises(tv.HeaderError, tv.write, out_f, [], {'id_string': 'not OK'})
    assert_raises(tv.HeaderError, tv.write, out_f, [], {'version': 3})
    assert_raises(tv.HeaderError, tv.write, out_f, [], {'hdr_size': 0})
...
def test_write_scalars_props():
    ...
    out_f = BytesIO()
    streams = [(points, None, None), (points, scalars, None)]
    assert_raises(tv.DataError, tv.write, out_f, streams)
    out_f.seek(0)
    streams = [(points, np.zeros((N, M + 1)), None), (points, scalars, None)]
    assert_raises(tv.DataError, tv.write, out_f, streams)
    # Or if scalars different N compared to points
    bad_scalars = np.zeros((N + 1, M))
    out_f.seek(0)
    streams = [(points, bad_scalars, None), (points, bad_scalars, None)]
    assert_raises(tv.DataError, tv.write, out_f, streams)
    # Similarly properties must have the same length for each streamline
    out_f.seek(0)
    streams = [(points, scalars, None), (points, scalars, props)]
    assert_raises(tv.DataError, tv.write, out_f, streams)
    out_f.seek(0)
    streams = [(points, scalars, np.zeros((P + 1, ))), (points, scalars,
                                                        props)]
    assert_raises(tv.DataError, tv.write, out_f, streams)
    # If all is OK, then we get back what we put in
    out_f.seek(0)
    streams = [(points, scalars, props), (points, scalars, props)]
    ...
    # issue #53)
    out_f.seek(0)
    streams = [(points.astype('f4'), scalars.astype('f4'), props.astype('f4'))]
    tv.write(out_f, streams)
    out_f.seek(0)
    back_streams, hdr = tv.read(out_f)
    for actual, expected in zip(streams, back_streams):
...
def test_points_processing():
    ...
    vxmm_streams = [(ijk0 * [[2, 3, 4]], None, None), (ijk1 * [[2, 3, 4]],
                                                       None, None)]
    ...
    for hdr in (  # these cause read / write errors
        ...
        out_f.seek(0)
        assert_raises(tv.HeaderError, tv.write, out_f, vx_streams, hdr, None,
                      'voxel')
        out_f.seek(0)
    ...
    for hdr in (  # all these cause read and write errors for rasmm
        ...
        # Check error on write
        out_f.seek(0)
        assert_raises(tv.HeaderError, tv.write, out_f, rasmm_streams, hdr,
                      None, 'rasmm')
        out_f.seek(0)
        # bypass write error and check read
        tv.write(out_f, vxmm_streams, hdr, None, points_space=None)
    ...
    # This should be OK
    hdr = {'voxel_size': [2, 3, 4], 'voxel_order': 'RAS', 'vox_to_ras': aff}
    ...
    fancy_affine = np.array([[0., -2, 0, 10], [3, 0, 0, 20], [0, 0, 4, 30],
                             [0, 0, 0, 1]])
    hdr = {
        'voxel_size': [3, 2, 4],
        'voxel_order': 'ALS',
    ...
    (raw_streams, hdr), (proc_streams, _) = _rt(fancy_rasmm_streams, hdr,
                                                'rasmm')
...
def test_empty_header():
    for endian in '<>':
        for version in (1, 2):
            ...
            assert_array_equal(hdr['image_orientation_patient'],
                               [0, 0, 0, 0, 0, 0])
...
def test_get_affine():
    ...
    assert_array_equal(old_afh(hdr), np.diag([0, 0, 0, 1]))
    hdr['voxel_size'] = 1
    assert_array_equal(old_afh(hdr), np.diag([0, 0, 0, 1]))
    # DICOM direction cosines
    hdr['image_orientation_patient'] = [1, 0, 0, 0, 1, 0]
    assert_array_equal(old_afh(hdr), np.diag([-1, -1, 1, 1]))
    # RAS direction cosines
    hdr['image_orientation_patient'] = [-1, 0, 0, 0, -1, 0]
    assert_array_equal(old_afh(hdr), np.eye(4))
    # translations
    hdr['origin'] = [1, 2, 3]
    exp_aff = np.eye(4)
    exp_aff[:3, 3] = [-1, -2, 3]
    ...
    for in_aff, o_codes in ((unflipped_aff, 'RAS'), (flipped_aff, 'RAI')):
        hdr = tv.empty_header()
...
def test_tv_class():
    ...
    out_f.truncate(0)
    out_f.seek(0)
    assert_raises(tv.HeaderError, tv.TrackvisFile, [], {'id_string': 'not OK'})
    assert_raises(tv.HeaderError, tv.TrackvisFile, [], {'version': 3})
    assert_raises(tv.HeaderError, tv.TrackvisFile, [], {'hdr_size': 0})
    affine = np.diag([1, 2, 3, 1])
    affine[:3, 3] = [10, 11, 12]
    ...
    assert_raises(tv.TrackvisFileError, tv.TrackvisFile, iter([]))
...
def test_tvfile_io():
    ...
    tvf = tv.TrackvisFile(
        vx_streams, points_space='voxel', affine=np.diag([2, 3, 4, 1]))
    tvf.to_file(out_f)
    out_f.seek(0)
    tvf2 = tv.TrackvisFile.from_file(out_f, points_space='voxel')
    assert_true(streamlist_equal(vx_streams, tvf2.streamlines))
    ...
    tvf = tv.TrackvisFile(
        fancy_rasmm_streams, points_space='rasmm', affine=fancy_affine)
    tvf.to_file(out_f)
    out_f.seek(0)
    tvf2 = tv.TrackvisFile.from_file(out_f, points_space='rasmm')
    assert_true(streamlist_equal(fancy_rasmm_streams, tvf2.streamlines))
