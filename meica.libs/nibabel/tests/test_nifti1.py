...
class TestNifti1PairHeader(tana.TestAnalyzeHeader):
    ...
    def test_nifti_log_checks(self):
        ...
        assert_equal(message, 'Unused "scl_inter" is 3.0; should be 0; '
        ...
        fhdr, message, raiser = self.log_chk(hdr, 20)
        assert_equal(fhdr['scl_inter'], 0)
        assert_equal(message, 'Unused "scl_inter" is %s; should be 0; '
        ...
        assert_equal(fhdr['scl_inter'], 0)
        assert_equal(message, '"scl_slope" is 1.0; but "scl_inter" is %s; '
        ...
        assert_equal(fhdr['scl_inter'], 0)
        assert_equal(message, '"scl_slope" is nan; should be finite; '
        ...
        assert_equal(message, 'qform_code -1 not valid; ' 'setting to 0')
        hdr = HC()
        hdr['sform_code'] = -1
        fhdr, message, raiser = self.log_chk(hdr, 30)
        assert_equal(fhdr['sform_code'], 0)
        assert_equal(message, 'sform_code -1 not valid; ' 'setting to 0')
...
class TestNifti1Image(tana.TestAnalyzeImage):
    ...
    def test_set_qform(self):
        img = self.image_class(
            np.zeros((2, 3, 4)), np.diag([2.2, 3.3, 4.3, 1]))
        hdr = img.get_header()
...
def test_datatypes():
    ...
    for code in data_type_codes.value_set():
        ...
        (assert_equal, hdr.get_data_dtype(), data_type_codes.dtype[code])
...
def test_intents():
    ...
    assert_equal(ehdr.get_intent(), ('t test', (10.0, ), 'some score'))
    # invalid intent name
...
def test_extension_codes():
    for k in list(extension_codes.keys()):
...
def test_slope_inter():
    ...
    for intup, outup in (((2.0, ), (2.0, 0.0)), ((None, ), (None, None)),
                         ((3.0, None), (3.0, 0.0)), ((0.0, None), (None,
                                                                   None)),
                         ((None, 0.0),
                          (None, None)), ((None, 3.0),
                                          (None, None)), ((2.0, 3.0), (2.0,
                                                                       3.0))):
