...
class TestAnalyzeHeader(_TestWrapStructBase):
    ...
    def test_checks(self):
        ...
        hdr['datatype'] = 0
        assert_equal(
            self._dxer(hdr), 'data code 0 not supported\n'
    ...
    def test_data_dtype(self):
        ...
        supported_types = ((2, np.uint8), (4, np.int16), (8, np.int32),
                           (16, np.float32), (32, np.complex64), (64,
                                                                  np.float64),
                           (128,
    ...
    def test_read_write_data(self):
        ...
        assert_raises(HeaderDataError, hdr.data_to_fileobj, np.zeros(3),
                      str_io)
        # Test valid write
        hdr.set_data_shape((1, 2, 3))
    ...
    def test_data_shape_zooms_affine(self):
        ...
        for shape in ((1, 2, 3), (0, ), (1, ), (1, 2), (1, 2, 3, 4)):
            ...
            if len(shape):
                assert_raises(HeaderDataError, hdr.set_zooms, (1, ) * (L - 1))
                # Errors for negative zooms
                assert_raises(HeaderDataError, hdr.set_zooms,
                              (-1, ) + (1, ) * (L - 1))
            assert_raises(HeaderDataError, hdr.set_zooms, (1, ) * (L + 1))
            # Errors for negative zooms
            assert_raises(HeaderDataError, hdr.set_zooms, (-1, ) * L)
    ...
    def test_base_affine(self):
        ...
        hdr.set_zooms((3, 2, 1))
        assert_true(hdr.default_x_flip)
        assert_array_almost_equal(hdr.get_base_affine(),
                                  [[-3., 0., 0., 3.], [0., 2., 0., -4.],
                                   [0., 0., 1., -3.], [0., 0., 0., 1.]])
...
def test_slope_inter():
    ...
    for slinter in ((None, ), (None, None), (1.0, ), (1.0, None), (None, 0),
                    (1.0, 0)):
