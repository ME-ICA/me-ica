...
from ..volumeutils import (
    array_from_file, array_to_file, calculate_scale, can_cast, write_zeros,
    apply_read_scaling, _inter_type, working_type, best_write_scale_ftype,
    better_float_of, int_scinter_ftype, allopen, make_dt_codes, native_code,
    shape_zoom_affine, rec2dict)
...
def test_apply_scaling():
    ...
    assert_equal(
        apply_read_scaling(np.int8(0), f32(-1.0), f32(0.0)).dtype, np.float32)
    ...
    assert_equal(
        apply_read_scaling(np.int8(0), f32(1e38), f32(0.0)).dtype, np.float64)
    assert_equal(
        apply_read_scaling(np.int8(0), f32(-1e38), f32(0.0)).dtype, np.float64)
    # Test that integer casting during read scaling works
    assert_dt_equal(apply_read_scaling(i16_arr, 1.0, 1.0).dtype, np.int32)
    assert_dt_equal(
        apply_read_scaling(np.zeros((1, ), dtype=np.int32), 1.0, 1.0).dtype,
        np.int64)
    assert_dt_equal(
        apply_read_scaling(np.zeros((1, ), dtype=np.int64), 1.0, 1.0).dtype,
        best_float())
    ...
    for first in FLOAT_TYPES:
        for other in IUINT_TYPES + np.sctypes['complex']:
            ...
            for other2 in IUINT_TYPES + np.sctypes['complex']:
                ...
                assert_equal(
                    better_float_of(other, other2, np.float64), np.float64)
        for second in FLOAT_TYPES:
            assert_equal(
                better_float_of(first, second), check_against(first, second))
...
def test_best_write_scale_ftype():
    ...
    for dtt in IUINT_TYPES + FLOAT_TYPES:
        ...
        assert_equal(
            best_write_scale_ftype(arr, 1, 0), better_float_of(
                dtt, np.float32))
        assert_equal(
            best_write_scale_ftype(arr, 1, 0, np.float64),
            better_float_of(dtt, np.float64))
        assert_equal(
            best_write_scale_ftype(arr, np.float32(2), 0),
            better_float_of(dtt, np.float32))
        assert_equal(
            best_write_scale_ftype(arr, 1, np.float32(1)),
            better_float_of(dtt, np.float32))
    ...
    for lower_t, higher_t in best_vals:
        ...
        assert_equal(
            best_write_scale_ftype(arr, 1, -big_delta / 2.01), lower_t)
        assert_equal(
            best_write_scale_ftype(arr, 1, -big_delta / 2.0), higher_t)
...
def test_can_cast():
    tests = (
        (np.float32, np.float32, True, True, True),
...
def test_shape_zoom_affine():
    ...
    exp = np.array([[-3., 0., 0., 3.], [0., 2., 0., -4.], [0., 0., 1., -3.],
                    [0., 0., 0., 1.]])
    assert_array_almost_equal(res, exp)
    res = shape_zoom_affine((3, 5), (3, 2))
    exp = np.array([[-3., 0., 0., 3.], [0., 2., 0., -4.], [0., 0., 1., -0.],
                    [0., 0., 0., 1.]])
    assert_array_almost_equal(res, exp)
...
def test_dtypes():
    ...
    assert_equal(dtr.fields, ('code', 'label', 'type', 'dtype', 'sw_dtype'))
    ...
    assert_equal(dtr[np.dtype('f4').newbyteorder('S')], 16)
    assert_equal(dtr.value_set(), set((16, )))
    assert_equal(dtr.fields,
                 ('code', 'label', 'type', 'niistring', 'dtype', 'sw_dtype'))
    assert_equal(dtr.niistring[16], 'ASTRING')
    # And that unequal elements raises error
    dt_defs = ((16, 'float32', np.float32, 'ASTRING'), (16, 'float32',
                                                        np.float32))
