...
def selcomps(seldict,
    ...
    #Temporal features
    mmix_kurt_z = (mmix_kurt - mmix_kurt[group0].mean()
                   ) / mmix_kurt[group0].std()  #larger is worse - spike
    mmix_std_z = -1 * (
    ...
    for t2sl_i in range(len(t2s_lim)):
        ...
        invein = veinW.sum(1)[fmask(
            unmask(veinmaskf, mask) * unmask(veinW.sum(1) > 1, mask), mask)]
    ...
    field_art = np.union1d(
        np.setdiff1d(nc[andb([
            ...
            Rhos > np.percentile(Rhos[group0], 75)
        ]) == 3], group0), field_art)
    field_art = np.union1d(
        np.setdiff1d(nc[andb([mmix_kurt_z_max > 5, Vz2 > 5]) == 2], group0),
        field_art)
