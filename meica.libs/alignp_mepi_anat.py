...
parser.add_option(
    '-p', "", dest='prefix', help="Alignment matrix prefix", default='')
parser.add_option(
    '',
    "--cmass",
    action='store_true',
...
parser.add_option(
    ...
    "--cost",
    dest='cost',
    help="Cost function, default -lpc",
    default='-lpc')
...
def import_datasets(dsets):
    ...
    for dset in dsets:
        if dset != '' and dset != None:
            ...
            sl.append("3dcopy -overwrite %s/%s %s/%s/%s" %
                      (indir, dset, startdir, walignp_dirname,
                       dsprefix(os.path.basename(dset))))
            sl.append("3drefit -view orig `ls %s/%s/%s+*.HEAD`" %
                      (startdir, walignp_dirname,
                       dsprefix(os.path.basename(dset))))
...
def align_centers(base, dset):
    sl.append("@Align_Centers -base %s+orig -dset %s+orig" % (dsprefix(base),
                                                              dsprefix(dset)))
...
def graywt(t2sname, s0name):
    ...
    sl.append("3dBrickStat -mask %s -percentile 50 1 50 %s > graywt_thr.1D" %
              (t2sname, t2sname))
    sl.append(
    ...
    sl.append("3dSeg -prefix Segsy.t2svm -anat %s -mask %s" % (basevol,
                                                               basevol))
    sl.append(
...
def allineate(sourcevol, weight, targetvol, prefix, maxrot, maxshf, maxscl,
    ...
    align_opts = "%s -weight %s -maxshf %s -maxrot %s -maxscl %s %s" % (
        options.cost, weightvol, maxrot, maxshf, maxscl, cmass_opt)
...
def cmselect(base, allin):
    if options.autocmass:
        ...
        sl.append(
            ...
            % (base, allin, allin))
        sl.append("echo `3ddot -dodice %s rs_ncm_%s` > ncm_olap.txt" % (base,
                                                                        allin))
        sl.append("echo `3ddot -dodice %s rs_%s` > cm_olap.txt" % (base,
                                                                   allin))
...
"""Import datasets"""
t2s_name, s0_name, anat_name = import_datasets(
    [options.t2s, options.s0, options.anat])
...
allin_volume, allin_matrix = allineate(
    anat_name, weightvol, basevol, options.prefix, options.maxrot,
    options.maxshf, options.maxscl, options.cmass)
