...
def dsprefix(idn):
    ...
    if len(split(idn, '.')) != 0:
        if split(idn,
                 '.')[-1] == 'HEAD' or split(idn, '.')[-1] == 'BRIK' or split(
                     idn, '.')[-2:] == ['BRIK', 'gz']:
            return prefix(idn)
        elif split(idn,
                   '.')[-1] == 'nii' and not split(idn, '.')[-1] == 'nii.gz':
...
@Gooey(
    ...
    required_cols=1,
    optional_cols=1,
    program_name="Multi-Echo ICA v3.2",
    default_size=(800, 650),
    monospace_display=True)
def main():
    #Configure options and help dialog
    parser = GooeyParser(
        description='Enter dataset configuration for basic ME-ICA processing.')
    ...
    parser.add_argument(
        ...
        "--echoes",
        dest='TEs',
        help="Comma separated TEs in units of ms. Ex: 14.5,38.5,62.5",
        default='')
    parser.add_argument(
        '--data',
        "--data",
        dest='ME-EPI Datasets',
        help="Choose multiple: e1.nii.gz:e2.nii.gz:e3.nii.gz",
        nargs='+',
    ...
    parser.add_argument(
        ...
        "--anat",
        dest='Anatomical',
        help="Example:   anat.nii.gz",
        widget="FileChooser",
        default='')
    parser.add_argument(
        '--basetime',
        "--basetime",
        dest='Equilibration Time',
        help="Example (s for sec, v for vol):   10s OR 10v",
        default='10s')
    parser.add_argument(
        '--MNI',
        "--MNI",
        dest='MNI Normalization',
        choices=['None', 'Affine', 'Nonlinear (QWarp)'],
        help="Warp to MNI space using high-resolution template",
        default='None')
    parser.add_argument(
        '-f',
        "--fres",
        dest='EPI Upsampling',
        help="In mm. Leave blank for none.  Example:   2.5mm",
        default='2.5mm')
    parser.add_argument(
        '-S',
        "--no_skullstrip",
        action="store_true",
        dest='Disable Automatic Anatomical Skullstrip',
        help=
        "Anatomical is already skull-stripped and intensity-normalized and skull-stripped",
        default=False)
    parser.add_argument(
        '-R',
        "--report",
        action="store_true",
        dest='Generate ME-ICA Report',
        help="Generate HTML report of ME-ICA components and denoising",
        default=False)
    parser.add_argument(
        '--native',
        "--native",
        action="store_true",
        dest='Native Space Outputs',
        help=
        "Output result datasets in native space in addition to standard space",
        default=False)
...
if __name__ == '__main__':
    ...
    if not ready:
        print(
            "Dependencies are not OK. Try downloading AFNI and setting on the path, and installing Anaconda Python distribution."
        )
    print()
    ...
    if ',' not in args.TEs:
        print(
            'ERROR: Please check the syntax of the comma-separated list of TEs'
        )
        print()
    args_TEs = args.TEs
    ...
    if len(theEPIs) < 3:
        print(
            'ERROR: Please select a ME-EPI dataset with >=3 image time series with unique filenames using the file chooser'
        )
        print()
    ...
    if len(theTEs) != len(theEPIs) and len(theEPIs) < 3:
        ...
        print(
            'ERROR: The number of TEs and the number of ME-EPI datasets do not match!'
        )
        print()
    ...
    natarg = ''
    anatarg = ''
    if vars(args)['Equilibration Time'] != '':
        btarg = '-b %s' % vars(args)['Equilibration Time']
    if theAnat != None:
        if vars(args)['MNI Normalization'] == 'Affine': mniarg = '--MNI'
        if vars(args)['MNI Normalization'] == 'Nonlinear (QWarp)':
            mniarg = '--MNI --qwarp'
        if vars(args)['Disable Automatic Anatomical Skullstrip']:
            nossarg = '--no_skullstrip'
        if vars(args)['Native Space Outputs']: natarg = '--native'
        anatarg = '-a %s' % theAnat
    if vars(args)['EPI Upsampling'] != '':
        fresarg = '--fres %s' % vars(args)['EPI Upsampling']

    print("RUNNING ME-ICA:")
    meica_cmd = "cd %s; %s %s/meica.py -e %s -d \"%s\" %s %s %s %s %s %s --OVERWRITE " % (
        outdir, sys.executable, meicadir, ','.join(theTEs), ','.join(theEPIs),
        anatarg, btarg, mniarg, fresarg, nossarg, natarg)
    sysrun(meica_cmd)
    print()
