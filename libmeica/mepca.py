...
from .fastica import install_dynamic_fastica
...
def tedpca(
    ...
    *,
    ...
    if len(ste) == 1 and ste[0] == -1:
        ...
        pcamask = makemask(assets.OCcatd[:, :, :, np.newaxis, :])
        d = fmask(assets.OCcatd, pcamask)
        ...
        ste = np.arange(ne)
        d = np.float64(fmask(catd, pcamask))
        eim = eimask(d) == 1
        d = d[eim]  # type: ignore
    else:
        ...
        d = np.float64(
            np.concatenate(
                [
                    fmask(catd[:, :, :, ee, :], pcamask)[:, np.newaxis, :]
                    for ee in ste - 1
                ],
                axis=1,
            )
    ...
    if not os.path.exists(pcastate_fn):
        ...
        eimum = np.array(np.squeeze(unmask(eimum.prod(1), pcamask)), dtype=bool)
        vTmix = v.T
    ...
    Kdaw_test = KR_V_thr_test(_K, _V)
    Kdaw_halfn = Kdaw_test[np.abs(Kdaw_test[:, 1] - min(100, len(_K) / 2)).argmin(), 0]

    if assets.kdaw < Kdaw_halfn and assets.kdaw == assets.kdaw_init:
        assets.kdaw = Kdaw_halfn
        print(f"Initial kdaw is too low, restarting at {assets.kdaw}")

    kappa_thr = KR_V_thr(_K, _V, assets.kdaw)

    _V_elb = getelbow(_V, True)
    _V_K_thr = _K[_V > _V_elb].min()
    if kappa_thr > _V_K_thr:
        kappa_thr = np.mean([_V_K_thr, kappa_thr])
...
def tedica(dd, nc, *, cost, assets):
    ...
    #
    # pudb.set_trace()
    FastICA = install_dynamic_fastica(n_attempts=2, seed=assets.rng_seed)
    icanode = FastICA(
        n_components=nc,
        algorithm="parallel",
        fun="logcosh",
