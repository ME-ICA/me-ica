...
parser.add_option(
    "", "--maxrot", dest="maxrot", help="Maximum rotation, default 30", default="30"
)
parser.add_option(
    "", "--maxshift", dest="maxshf", help="Maximum shift, default 30", default="30"
)
parser.add_option(
    "", "--maxscl", dest="maxscl", help="Maximum scale, default 1.01", default="1.01"
)
parser.add_option(
    "", "--cost", dest="cost", help="Cost function, default -lpc", default="-lpc"
)
        ...
        if dset != "" and dset != None:
            ...
            sl.append(
                ...
                % (
                    ...
                    startdir,
                    walignp_dirname,
                    dsprefix(os.path.basename(dset)),
                )
            )
            sl.append(
                "3drefit -view orig `ls %s/%s/%s+*.HEAD`"
                % (startdir, walignp_dirname, dsprefix(os.path.basename(dset)))
            )
...
def align_centers(base, dset):
    sl.append(
        "@Align_Centers -base %s+orig -dset %s+orig" % (dsprefix(base), dsprefix(dset))
    )
...
def graywt(t2sname, s0name):
    ...
    if s0name != "":
        sl.append(
            "3dcalc -overwrite -a %s -b %s -expr 'a*ispositive(a)*step(b)' -prefix t2svm_ss.nii.gz"
            % (t2sname, s0name)
        )
        t2sname = "t2svm_ss.nii.gz"
    sl.append(
        "3dBrickStat -mask %s -percentile 50 1 50 %s > graywt_thr.1D"
        % (t2sname, t2sname)
    )
    sl.append("maxthr=`cat graywt_thr.1D`; maxthra=($maxthr); vmax=${maxthra[1]}")
    sl.append(
        '3dcalc -overwrite -a %s -expr "a*isnegative(a-2*${vmax})" -prefix t2svm_thr.nii.gz'
        % (t2sname)
    )
    ...
    sl.append(
        "3dcalc -overwrite -a 'Segsy.t2svm/Posterior+orig[2]' -prefix %s -expr 'a' "
        % weightvol
    )
...
def allineate(sourcevol, weight, targetvol, prefix, maxrot, maxshf, maxscl, do_cmass):
    ...
    cmass_opt = ""
    autocmass = False
    if do_cmass or options.autocmass:
        cmass_opt = "-cmass"
    elif not do_cmass and not options.autocmass:
        cmass_opt = ""
    align_opts = "%s -weight %s -maxshf %s -maxrot %s -maxscl %s %s" % (
        options.cost,
        weightvol,
        maxrot,
        maxshf,
    ...
    sl.append(
        "3dAllineate -overwrite -weight_frac 1.0 -VERB -warp aff -source_automask+2 -master SOURCE -source %s -base %s -prefix ./%s -1Dmatrix_save ./%s %s "
    ...
    sl.append(
        "3dAllineate -overwrite -weight_frac 1.0 -VERB -warp aff -source_automask+2 -master SOURCE -source %s -base %s -prefix ./ncm_%s -1Dmatrix_save ./ncm_%s %s "
        % (sourcevol, targetvol, outvol_prefix, outmat_prefix, align_opts)
    )
...
def cmselect(base, allin):
    if options.autocmass:
        sl.append(
            "3dresample -overwrite -master %s -prefix rs_%s -inset %s -rmode NN"
            % (base, allin, allin)
        )
        sl.append(
            "3dresample -overwrite -master %s -prefix rs_ncm_%s -inset ncm_%s -rmode NN"
            % (base, allin, allin)
        )
        ...
        sl.append(
            """if [ "$mv_dec" == "1" ] ; then 3drename -overwrite ncm_%s %s ; mv ncm_%s_mat.aff12.1D %s_mat.aff12.1D; fi """
            % (allin, allin, dsprefix(allin), dsprefix(allin))
        )
...
"""Run 3dAllineate"""
allin_volume, allin_matrix = allineate(
    anat_name,
    weightvol,
    basevol,
    options.prefix,
