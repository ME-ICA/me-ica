...
def optcom(data, t2s, tes, mask, assets=None):
    """
    out = optcom(data,t2s)
    ...
    if assets is not None:
        combmode = assets.options.combmode
    else:
        try:
...
if __name__ == "__main__":
    ...
    if options.label != None:
        s, exituf = "_%s" % str(options.label)
    else:
        suf = ""
    ...
    aff = catim.affine
    catd = cat2echos(catim.get_fdata(), ne)  # mention change from get_data to get_fdata
    nx, ny, nz, Ne, nt = catd.shape
    mu = catd.mean(axis=-1)
    sig = catd.std(axis=-1)
