...
def dep_check():
    ...
    if sys.version_info < (3, 6):
        print(
            "*+ Python 3.x is <3.6, please upgrade to Python 3.x >= 3.6 & Numpy >= 1.5.x."
        )
        fails += 1
    else:
        python_version_ok = 1
    ...
    if numpy_installed:
        ...
        if version_checker(numpy.__version__, 1.5) == False:
            fails += 1
            print(
                "*+ Numpy version is too old! Please upgrade to Numpy >=1.5.x!"
            )
        import numpy.__config__ as nc
        if nc.blas_opt_info == {}:
            fails += 1
            print(
                "*+ Numpy is not linked to BLAS! Please check Numpy installation."
            )
    if scipy_installed:
        ...
        if version_checker(scipy.__version__, 0.11) == False:
            fails += 1
            print(
                "*+ Scipy version is too old! Please upgrade to Scipy >=0.11.x!"
            )
    ...
    if afnicheck[0] != 0:
        ...
        fails += 1
    elif not afnicheck[1].__contains__('Alternate Alternative Usage') and len(
            afnisegcheck[1]) < 1000:
        print(
...
parser.add_option(
    ...
    "",
    dest='tes',
    help="ex: -e 14.5,38.5,62.5  Echo times (in ms)",
    default='')
parser.add_option(
    '-d',
    "",
...
parser.add_option(
    ...
    dest='basetime',
    help=
    "ex: -b 10s OR -b 10v  Time to steady-state equilibration in seconds(s) or volumes(v). Default 0. ",
    default='0')
parser.add_option(
    '',
    "--MNI",
...
extopts.add_option(
    ...
    dest='space',
    help=
    "Path to specific standard space template for affine anatomical normalization",
    default=False)
extopts.add_option(
    '',
    "--fres",
    dest='fres',
    help=
    "Specify functional voxel dim. in mm (iso.) for resampling during preprocessing. Default none. ex: --fres=2.5",
    default=False)
extopts.add_option(
    '',
    "--no_skullstrip",
    ...
    dest='no_skullstrip',
    help=
    "Anatomical is already intensity-normalized and skull-stripped (if -a provided)",
    default=False)
extopts.add_option(
    '',
    "--no_despike",
    ...
    dest='no_despike',
    help=
    "Do not de-spike functional data. Default is to de-spike, recommended.",
    default=False)
extopts.add_option(
    '',
    "--no_axialize",
    ...
    dest='no_axialize',
    help=
    "Do not re-write dataset in axial-first order. Default is to axialize, recommended.",
    default=False)
extopts.add_option(
    '',
    "--mask_mode",
    dest='mask_mode',
    help=
    "Mask functional with help from anatomical or standard space images: use 'anat' or 'template' ",
    default='func')
extopts.add_option(
    '',
    "--coreg_mode",
    dest='coreg_mode',
    help=
    "Coregistration with Local Pearson and T2* weights (default), or use align_epi_anat.py (edge method): use 'lp-t2s' or 'aea'",
    default='lp-t2s')
...
extopts.add_option(
    ...
    dest='tpattern',
    help=
    "Slice timing (i.e. alt+z, see 3dTshift -help). Default from header. (N.B. This is important!)",
    default='')
extopts.add_option(
    '',
    "--align_args",
    dest='align_args',
    help=
    "Additional arguments to anatomical-functional co-registration routine",
    default='')
extopts.add_option(
    '',
    "--ted_args",
...
extopts.add_option(
    '', "--tlrc", dest='space', help=SUPPRESS_HELP,
    default=False)  #For backwards compat. with existing scripts
extopts.add_option(
    '', "--highpass", dest='highpass', help=SUPPRESS_HELP, default=0.0)
extopts.add_option(
    '', "--detrend", dest='detrend', help=SUPPRESS_HELP, default=0.)
extopts.add_option(
...
runopts.add_option(
    ...
    action='store_true',
    help=
    "Attempt to resume from normalization step onwards, overwriting existing")
runopts.add_option(
    '',
    "--OVERWRITE",
    dest='overwrite',
...
runcmd = " ".join(sys.argv).replace(options.dsinputs,
                                    r'"%s"' % options.dsinputs).replace(
                                        '"', r'"')
...
for ds_ii in range(len(datasets)):
    if subprocess.getstatusoutput('3dinfo %s' % (getdsname(ds_ii)))[0] != 0:
        ...
        notfound += 1
if options.anat != '' and subprocess.getstatusoutput('3dinfo %s' %
                                                     (options.anat))[0] != 0:
...
if options.TR != '': tr = float(options.TR)
    ...
    tr = float(
        os.popen('3dinfo -tr %s' % (getdsname(0))).readlines()[0].strip())
    options.TR = str(tr)
...
if options.anat != '':
    oblique_anat_read = int(
        os.popen('3dinfo -is_oblique %s' %
                 (options.anat)).readlines()[0].strip())
    epicm = [
        float(coord) for coord in os.popen("3dCM %s" % (
            getdsname(0))).readlines()[0].strip().split()
    ]
    anatcm = [
        float(coord) for coord in os.popen("3dCM %s" % (
            options.anat)).readlines()[0].strip().split()
    ]
    maxvoxsz = float(
        os.popen("3dinfo -dk %s" % (getdsname(0))).readlines()[0].strip())
    deltas = [
    ...
    cmdif = max(
        abs(epicm[0] - anatcm[0]), abs(epicm[1] - anatcm[1]),
        abs(epicm[2] - anatcm[2]))
    addslabs = abs(int(cmdif / maxvoxsz)) + 10
    zeropad_opts = " -I %s -S %s -A %s -P %s -L %s -R %s " % (
        tuple([addslabs] * 6))
...
if options.anat == '' and options.mask_mode != 'func':
    print(
        "*+ Can't do anatomical-based functional masking without an anatomical!"
    )
    sys.exit()
...
if " -affter aaa  = *** THIS OPTION IS NO LONGER AVAILABLE" in subprocess.getstatusoutput(
        "3dNwarpApply -help")[1]:
    old_qwarp = False
else:
    old_qwarp = True
...
if options.anat != '':
    ...
    if not options.no_skullstrip:
        sl.append(
            ...
            % (pathanatprefix, pathanatprefix, startdir, nsmprage,
               pathanatprefix, pathanatprefix, pathanatprefix, pathanatprefix))
...
for e_ii in range(len(datasets)):
    ds = datasets[e_ii]
    sl.append("3dcalc -a %s/%s -expr 'a' -prefix ./%s.nii" %
...
if oblique_mode:
    if options.anat != '':
        sl.append(
            ...
            % (vrAinput, anatprefix, startdir, nsmprage, prefix))
    else:
        sl.append("3dWarp -overwrite -prefix %s -deoblique %s" % (vrAinput,
                                                                  vrAinput))
#Despike and axialize
if not options.no_despike:
    sl.append("3dDespike -overwrite -prefix ./%s_vrA%s %s " % (vrbase, osf,
                                                               vrAinput))
    vrAinput = "./%s_vrA%s" % (vrbase, osf)
if not options.no_axialize:
    sl.append("3daxialize -overwrite -prefix ./%s_vrA%s %s" % (vrbase, osf,
                                                               vrAinput))
    vrAinput = "./%s_vrA%s" % (vrbase, osf)
#Set eBbase
...
for echo_ii in range(len(datasets)):
    ...
    if not options.no_despike:
        ...
        sl.append("3dDespike -overwrite -prefix %s %s%s" % (intsname,
                                                            dsprefix(indata),
                                                            isf))
    #Time shift datasets
    if options.tpattern != '':
        ...
        tpat_opt = ''
    sl.append("3dTshift -heptic %s -prefix ./%s_ts+orig %s" % (tpat_opt, dsin,
                                                               intsname))
    ...
    if not options.no_axialize:
        sl.append("3daxialize  -overwrite -prefix ./%s_ts+orig ./%s_ts+orig" %
                  (dsin, dsin))
    if oblique_mode:
...
sl.append("%s %s -d basestack.nii.gz -e %s" %
          (sys.executable, '/'.join([meicadir, 'meica.libs', 't2smap.py']),
           options.tes))
...
if options.anat != '':
    ...
    logcomment(
        "Copy anatomical into ME-ICA directory and process warps", level=1)
    ...
    if options.space:
        ...
        sl.append(
            ...
            % (startdir, atnsmprage, options.space, dsprefix(nsmprage)))
        sl.append("cp %s.nii %s" % (dsprefix(atnsmprage), startdir))
        sl.append("gzip -f %s/%s.nii" % (startdir, dsprefix(atnsmprage)))
        sl.append("else if [ ! -e %s/%s ]; then ln -s %s/%s .; fi" %
                  (startdir, atnsmprage, startdir, atnsmprage))
        refanat = '%s/%s' % (startdir, atnsmprage)
        sl.append("fi")
        sl.append("3dcopy %s/%s.nii.gz %s" % (startdir, dsprefix(atnsmprage),
                                              dsprefix(atnsmprage)))
        sl.append("rm -f %s+orig.*; 3drefit -view orig %s+tlrc " %
        ...
        if options.qwarp:
            ...
            sl.append("3dUnifize -overwrite -GM -prefix ./%su.nii.gz %s/%s" %
            ...
            sl.append(
                ...
                % (options.space, startdir, dsprefix(atnsmprage),
                   dsprefix(atnsmprage)))
            sl.append("fi")
            sl.append("if [ ! -e %s/%s ]; then ln -s %s/%s .; fi" %
    ...
    if options.coreg_mode == 'lp-t2s':
        ...
        if not options.no_axialize:
            ...
            sl.append("3daxialize -overwrite -prefix ./%s %s" %
                      (ama_alnsmprage, alnsmprage))
        ...
        sl.append("cp alignp.mepi/mepi_al_mat.aff12.1D ./%s_al_mat.aff12.1D" %
                  anatprefix)
        ...
        sl.append(
            ...
            % (options.align_args))
        sl.append("cp ANAT_ns_al_mat.aff12.1D %s_al_mat.aff12.1D" %
                  (anatprefix))
    if options.space:
        tlrc_opt = "%s/%s::WARP_DATA -I" % (startdir, atnsmprage)
        inv_tlrc_opt = "%s/%s::WARP_DATA" % (startdir, atnsmprage)
        sl.append("cat_matvec -ONELINE %s > %s/%s_xns2at.aff12.1D" %
                  (tlrc_opt, startdir, anatprefix))
        sl.append("cat_matvec -ONELINE %s > %s_xat2ns.aff12.1D" %
                  (inv_tlrc_opt, anatprefix))
    else:
        tlrc_opt = ""
...
sl.append(
    "3dBrickStat -mask eBbase.nii.gz -percentile 50 1 50 %s_ts+orig[%i] > gms.1D"
    % (e1_dsin, basebrik))
...
for echo_ii in range(len(datasets)):
    ...
    if echo_ii == 0:
        ...
        sl.append("3dZeropad %s -prefix eBvrmask.nii.gz ocv_ss.nii.gz[0]" %
                  (zeropad_opts))
        #Create base mask
        #if valid_qwarp_mode:
        ...
        if options.anat and options.mask_mode != 'func':
            ...
            if options.fres:
                resstring = "-dxyz %s %s %s" % (options.fres, options.fres,
                                                options.fres)
            else:
        ...
        sl.append("3dAutobox -overwrite -prefix eBvrmask%s eBvrmask%s" % (osf,
                                                                          osf))
        resstring = "-dxyz ${voxsize} ${voxsize} ${voxsize}"
        sl.append(
    ...
    if echo_ii == 0:
        sl.append("3dTstat -min -prefix ./%s_vr_min%s ./%s_vr%s" % (dsin, osf,
                                                                    dsin, osf))
        sl.append(
    ...
    if options.highpass:
        sl.append("3dBandpass -prefix ./%s_in%s %f 99 ./%s_sm%s " %
    ...
    sl.append("3dTstat -stdev -prefix ./%s_std%s ./%s_in%s" % (dsin, osf, dsin,
                                                               osf))

    if options.test_proc: sl.append("exit")
    if not (options.test_proc or options.keep_int):
        sl.append("rm -f %s_pt.nii.gz %s_vr%s %s_sm%s" % (dsin, dsin, osf,
                                                          dsin, osf))
...
sl.append("3dcalc -float -overwrite -a %s[0] -expr 'notzero(a)' -prefix %s" %
          (ica_input, ica_mask))
...
sl.append(
    ...
    % (tedflag, sys.executable, interactive_flag,
       '/'.join([meicadir, tedanapath]), options.tes, ica_input,
       options.sourceTEs, options.daw, options.initcost, options.finalcost,
       strict_setting, options.ted_args))
if outprefix == '': outprefix = setname

if options.select_only:
    sl = []
    sl.append(
        "%s %s %s -e %s  -d %s --sourceTEs=%s --kdaw=%s --rdaw=1 --initcost=%s --finalcost=%s --conv=2.5e-5 --mix=meica_mix.1D %s %s "
        % (sys.executable, interactive_flag, '/'.join([meicadir, tedanapath]),
           options.tes, ica_input, options.sourceTEs, options.daw,
           options.initcost, options.finalcost, strict_setting,
           options.ted_args))

mask_dict = {}  #Need this here


def export_result(infile,
                  outfileprefix,
    ...
    if valid_qwarp_mode:
        ...
        this_nwarpstring = " -nwarp %s/%s_xns2at.aff12.1D '%s/%s_WARP.nii.gz' " % (
            startdir, anatprefix, startdir, dsprefix(nlatnsmprage))
        ...
        to_export.append(('%s_%s' % (outfileprefix, warp_code),
    ...
    if native_export:
        ...
        sl.append(
            ...
            % (tedflag, export_master, resstring, infile, outfileprefix,
               native_suffix))
        ...
        if not warp_code in list(mask_dict.keys()):
            ...
            sl.append(
                ...
                % (tedflag, export_master, resstring, 'export_mask.nii.gz',
                   warp_code))
            mask_dict[warp_code] = '%s_export_mask.nii' % warp_code
        to_export.append(('%s_%s' % (outfileprefix, warp_code),
                          '%s_export_mask.nii' % warp_code))
...
export_result(
    'TED/ts_OC.nii',
    '%s_tsoc' % (outprefix),
    "T2* weighted average of ME time series, produced by ME-ICA %s" %
    __version__,
...
export_result(
    'TED/betas_OC.nii',
    '%s_mefl' % (outprefix),
    "Full ICA coeff. set for component assessment, produced by ME-ICA %s" %
    __version__,
    disable=options.pp_only)
export_result(
    'TED/feats_OC2.nii',
    ...
    disable=options.pp_only)
export_result(
    'TED/comp_table.txt', '%s_ctab.txt' % (outprefix), plaintext=True)
