...
for echo_ii in range(len(datasets)):
    ...
    if not options.no_despike:
        ...
        sl.append(
            "3dDespike -overwrite -prefix %s %s%s" % (intsname, dsprefix(indata), isf)
        )
    # Time shift datasets
    if options.tpattern != "":
        ...
        tpat_opt = ""
    sl.append(
        "3dTshift -heptic %s -prefix ./%s_ts+orig %s" % (tpat_opt, dsin, intsname)
    )
...
sl.append(
    ...
    % (sys.executable, "/".join([meicadir, "meica.libs", "t2smap.py"]), options.tes)
)
...
if options.anat != "":
    ...
    if options.coreg_mode == "lp-t2s":
        ...
        sl.append(
            "cp alignp.mepi/mepi_al_mat.aff12.1D ./%s_al_mat.aff12.1D" % anatprefix
        )
    ...
    if options.space:
        ...
        sl.append(
            "cat_matvec -ONELINE %s > %s_xat2ns.aff12.1D" % (inv_tlrc_opt, anatprefix)
        )
    else:
        tlrc_opt = ""
    if oblique_mode:
...
for echo_ii in range(len(datasets)):
    ...
    if echo_ii == 0:
        ...
        if options.anat:
            almaster = "-master %s" % nsmprage  # abmprage
        else:
            almaster = ""
        # print 'almaster line is', almaster   #DEBUG
        # print 'refanat line is', refanat   #DEBUG
        sl.append(
            "3dZeropad %s -prefix eBvrmask.nii.gz ocv_ss.nii.gz[0]" % (zeropad_opts)
        )
        # Create base mask
        # if valid_qwarp_mode:
    ...
    if echo_ii == 0:
        sl.append(
            "3dTstat -min -prefix ./%s_vr_min%s ./%s_vr%s" % (dsin, osf, dsin, osf)
        )
        sl.append(
...
def export_result(
    ...
    if plaintext:
        esl.append("cp %s %s/%s;" % (infile, startdir, outfileprefix))
        return
